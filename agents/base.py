"""Agent base — LLM-powered (Gemini/Anthropic) and mock agents."""
...
def _build_game_prompt(player: Player, state: GameState) -> str:
    """Build the game state prompt (shared across LLM providers)."""
...
def _build_chat_prompt(player: Player, state: GameState, round_chat: list[dict]) -> str:
    """Build the info round chat prompt (shared across LLM providers)."""
    board = " ".join(str(c) for c in state.community_cards) if state.community_cards else "None"
    hole = " ".join(str(c) for c in player.hole_cards)
    ...
    return f"""INFO GATHERING ROUND — Talk before betting begins.
...
class GeminiAgent:
    """LLM-powered agent using Google Gemini (free tier)."""
    ...
    def model(self):
        if self._model is None:
            ...
            self._model = genai.GenerativeModel(
                ...
                system_instruction=f"You are {self.name} ({self.emoji}), a poker player.\n\n{self.strategy}\n\nStay in character. Be entertaining.",
            )
        return self._model
...
class LLMAgent:
    ...
    def decide(self, player: Player, state: GameState) -> AgentDecision:
        ...
        user = _build_game_prompt(player, state)
        try:
            response = self.client.messages.create(
                model=LLM_CONFIG["anthropic_model"],
